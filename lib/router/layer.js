...
function compilePath(path, keys, options) {
  ...
  if (shouldUseLegacyPathCompiler(path)) {
...
function shouldUseLegacyPathCompiler(path) {
  ...
  if (Array.isArray(path)) {
    for (var i = 0; i < path.length; i++) {
      if (shouldUseLegacyPathCompiler(path[i])) {
        return true;
      }
    }
  ...
  // Express 4 treats these as pattern operators, while v8 has different semantics.
  return /(^|[^\\])[?+*()[\]]/.test(path);
}
  ...
  if (path instanceof RegExp) {
    ...
    if (groups) {
      for (var i = 0; i < groups.length; i++) {
        keys.push({ name: i, optional: false, offset: 0 });
      }
    }
